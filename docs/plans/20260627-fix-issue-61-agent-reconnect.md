...
- **`processInitialDevices` add-only на реконнекте.** Снапшот `RegisterResponse` добавляет/обновляет онлайн-устройства, но не удаляет `onlineDevices`/mount'ы пиров, отсутствующих в снапшоте. Пир, ушедший offline в окне мёртвого стрима (его `DeviceOffline` пропущен), сохраняет mount на мёртвый endpoint — остаточный экземпляр сбоя #61. Полный reconcile (diff снапшота против `onlineDevices` + unmount отсутствующих) закрыл бы это, но рискует ложно отмонтировать пиров, которые просто ещё не ре-зарегистрировались после рестарта хаба (пустой in-memory реестр у хаба сразу после старта). Add-only — сознательный выбор: риск ложного unmount живого пира перевешивает хвостовой случай, который само-лечится при следующем `DeviceOffline`/`DeviceOnline`. Guarded reconcile НЕ делаем.
