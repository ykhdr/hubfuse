...
- [ ] `processInitialDevices`: удалить под `d.mu` записи `onlineDevices`, отсутствующие в
      снапшоте (снапшот авторитетен); ничего не размонтировать в этом пути
- [ ] докомментарий: почему unmount исчезнувших пиров отдан монитору (консервативность)
- [ ] тесты: пир, ушедший за время дохлого стрима, удалён из map после ре-регистрации;
      присутствующие обновлены; их маунты в этом пути не тронуты
- [ ] `go test ./internal/agent/...` — зелёные перед задачей 5
