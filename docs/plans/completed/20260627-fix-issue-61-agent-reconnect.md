...
- [x] обновить `CLAUDE.md`, если появился паттерн, достойный фиксации (например, «agent supervises hub session») — допилена строка про `daemon.go` в разделе «Agent internals»: супервизор сессии хаба переподключает Register→Subscribe с backoff (фикс issue #61)
- [x] README обновлять не требуется (внешнего поведения CLI не меняем)
- [x] перенести этот план в `docs/plans/completed/` — перенос выполнит оркестратор после всех фаз; вручную НЕ двигаем (иначе ломаются последующие review/finalize/stats фазы)
- [x] финальный коммит + пуш; открыть PR в `master` — сделан только локальный коммит; push/PR — вручную пользователем (нет GitHub-кредов в окружении)
...
- **Remount vs non-empty-refusal guard (только реальный FUSE):** при remount после *lazy* unmount (`fusermount -uz`/`umount -l`) обычный flow заново вызывает `targetHasLocalContents` (`mounter.go:299-310`); медленный lazy-teardown может на миг показать старое удалённое содержимое и споткнуться о «refusing to mount over non-empty dir». Стаб/юнит этого не ловят (`targetHasLocalContents` под stub возвращает 0). Само-лечится (stale-entry уже удалён → следующий `DeviceOnline`-эвент идёт обычным путём и домонтирует), но проверить на реальной машине.
...
### Codex review — мульти-share remount роуминг-пира (РЕАЛИЗОВАНО)

- **Проблема:** `shouldMount(nickname)` возвращал только ПЕРВЫЙ `MountConfig` для никнейма пира. Пир, экспортирующий несколько шар (каждая — отдельный mount-entry с одним `Device`), при `DeviceOnline`/реконнекте перемонтировал лишь одну шару; остальные оставались без mount, а после смены IP/порта — на протухшем endpoint. Цель #61 («пиры перемонтируют при смене endpoint») для мульти-share пира означает ВСЕ его шары.
- **Фикс:** добавлен `mountsForDevice(nickname) []MountConfig` (все совпадения по `Device`). `processInitialDevices` (реконнект-снапшот) и `handleDeviceOnline` (roam-эвент) теперь итерируют по ВСЕМ конфигам пира и зовут `Mount` для каждого — remount-ветка `Mount` (Task 2) переводит каждую шару на новый endpoint индивидуально. `shouldMount` переписан как тонкая обёртка над `mountsForDevice` (first-match) — его юнит-тесты остаются зелёными. Юнит-тесты: `TestHandleDeviceOnline_AutoMountsAllSharesOfPeer`, `TestProcessInitialDevices_AutoMountsAllSharesOfPeer` (обе: 2 шары монтируются + обе перемонтируют на новый IP).
- **`handlePairingCompleted` оставлен на `shouldMount` (одношаровый).** Это НЕ endpoint-change/roam-путь (одноразовое событие завершения пэйринга), он вне ядра #61, и так `shouldMount` сохраняет реального прод-вызывающего (не превращается в test-only хелпер). Хвостовой случай (мульти-share пир, уже онлайн в момент пэйринга, получает только первую шару сразу; остальные домонтируются на следующем `DeviceOnline`/реконнекте) — пред-существующий и само-лечится. `tryMount` (config-watcher) не затронут: он работает по конкретному mount-entry из `diff.MountsAdded`, а не по никнейму.
- **`unmountKey` doc-comment поправлен:** remount-путь (`Mount`, ветка смены endpoint) добавлен как валидный `reguard=false` вызывающий (помимо shutdown) — обычный mount-flow сразу сам re-guard'ит таргет.

### Review phase 1 — осознанные компромиссы (НЕ реализуем)

- **`processInitialDevices` add-only на реконнекте (device-level).** Снапшот `RegisterResponse` добавляет/обновляет онлайн-устройства, но не удаляет `onlineDevices`/mount'ы пиров, отсутствующих в снапшоте. Пир, ушедший offline в окне мёртвого стрима (его `DeviceOffline` пропущен), сохраняет mount на мёртвый endpoint — остаточный экземпляр сбоя #61. Полный reconcile (diff снапшота против `onlineDevices` + unmount отсутствующих) закрыл бы это, но рискует ложно отмонтировать пиров, которые просто ещё не ре-зарегистрировались после рестарта хаба (пустой in-memory реестр у хаба сразу после старта). Add-only — сознательный выбор: риск ложного unmount живого пира перевешивает хвостовой случай, который само-лечится при следующем `DeviceOffline`/`DeviceOnline`. Guarded reconcile НЕ делаем.
- **`mountsForOnlineDevice` add-only на реконнекте (share-level)** — тот же класс, но на уровне отдельной шары (codex iter3). `mountsForOnlineDevice` пересекает локальный конфиг с актуально экспортируемыми шарами пира (`info.Shares`), поэтому шару, которую пир перестал экспортировать, она НЕ перемонтирует — но и НЕ отмонтирует уже-активный mount этой шары. Если `SharesUpdated`-removal был пропущен в окне мёртвого стрима, протухшая шара остаётся смонтированной до следующего живого `SharesUpdated`/`DeviceOffline`. Reconcile (unmount активных mount'ов, чьих шар нет в `info.Shares`, прямо на реконнекте) НЕ делаем по той же причине, что и device-level: снапшот может временно не содержать шару, которую пир просто ещё не ре-анонсировал (пустой/частичный реестр хаба сразу после рестарта), и reconcile ложно отмонтировал бы живую шару. Хвостовой случай само-лечится при следующем живом `SharesUpdated`/`DeviceOffline`. Документируем, кода нет.
- **Remount расширяет worst-case удержание `m.mu`** с ~10s до ~15s (`unmountKey` ≤5s + mount-verify ≤10s — оба под локом в `Mount`). На shutdown `UnmountAllForce`→`unmountAll` берёт `m.mu` обычным `Lock()`, не подчинённым 5s shutdown-ctx (`mounter.go:653/670`), поэтому in-flight remount ровно в момент shutdown может задержать teardown до ~15s — за пределами 5s бюджета и 10s daemonize-SIGKILL дедлайна. Узкое предусловие (Mount должен идти ровно при shutdown; все call-site используют `context.Background()`, который shutdown не отменяет). 10s-часть (mount-verify под локом) пред-существующая; remount добавляет 5s. Полное исправление (вынести mount-verify из-под лока либо подчинить shutdown-ctx) затрагивает хрупкую bounded-teardown логику (#50/#47) и выходит за scope #61.
- **Реап backend-процесса при unmount — РЕАЛИЗОВАНО** (не компромисс, фикс): `unmountKey` теперь `Wait()`-ит `mnt.cmd` на обоих delete-путях (`reapMountCmd`). sshfs демонизируется (родитель выходит после монтирования), поэтому к моменту unmount `cmd` — давно завершённый зомби, `Wait()` мгновенный и не блокирует bounded teardown; без реапа долгоживущий роуминг-демон (#61 ремонтит на каждой смене endpoint) копил бы незажатые дочерние процессы.

### Codex iteration 3 — порядок config-reload/reconnect + endpoint-снапшот tryMount

- **`onConfigChange`: swap `d.config` ДО публикации шар (РЕАЛИЗОВАНО).** Раньше функция публиковала новые шары на хаб (`UpdateShares(new)`) и только в конце свопила `d.config = new`. Супервизор переподключения гоняет `sessionOnce` (Register) параллельно с config-watcher'ом; `sessionOnce` снапшотит `d.config` и ре-регистрирует его шары. В окне между `UpdateShares(new)` и поздним свопом конкурентный `sessionOnce` мог снапшотить СТАРЫЕ шары и зарегистрировать их ПОСЛЕ `UpdateShares(new)` — оставив хаб с протухшими шарами. Окно широкое (вся mount-обработка `onConfigChange`, включая FS-syscalls в `tryMount`/`Unmount`). Фикс: своп `d.config` перенесён в начало `onConfigChange` (сразу после `ComputeDiff`), так in-memory конфиг согласован с публикацией — любой `sessionOnce` после свопа снапшотит уже новые шары. Между новым местом свопа и публикацией ничего не читает `d.config`, поэтому перенос безопасен; лок не держится через блокирующий RPC. Публикация заведена через новый seam `updateSharesFn` (как `registerFn`/`subscribeFn`, т.к. `HubClient` без интерфейса). Юнит-тест: `TestOnConfigChange_SwapsConfigBeforePublishingShares` (seam ловит `d.config` ровно в момент публикации и проверяет, что это уже новый указатель).
- **`tryMount`: снапшот endpoint'а по значению (РЕАЛИЗОВАНО, hardening).** `tryMount` держал указатель `*OnlineDevice` из карты и читал `info.DeviceID/IP/SSHPort` уже после `RUnlock`. Теперь endpoint-примитивы (`deviceID/ip/sshPort`) копируются по значению под `RLock`, и `Mount` зовётся по локалам. Это устраняет алиасинг указателя на запись карты через разлоченный участок.
- **Заявленная DATA RACE в `tryMount` — ЛОЖНОЕ СРАБАТЫВАНИЕ.** Единственная in-place мутация поля `OnlineDevice` в коде — `info.Shares = newShares` в `handleSharesUpdated` (`events.go:137`); `IP/SSHPort/DeviceID/Nickname` НИКОГДА не мутируются на месте — `handleDeviceOnline` (`events.go:56`) и `processInitialDevices` (`daemon.go:675`) ЗАМЕНЯЮТ слот карты новым `*OnlineDevice`. `tryMount` читает только immutable-after-publish поля (endpoint), а не единственное мутируемое (`.Shares`) → гонки данных нет (подтверждено `-race`). Снапшот-по-значению применён как защита на будущее (если кто-то добавит in-place мутацию endpoint'а по образцу `.Shares`) и как очистка паттерна.
- **Остаточная логическая гонка remount-на-старый-endpoint в `tryMount` — пред-существующая, вне scope, само-лечится.** Сценарий: только что добавленный в конфиг mount-entry для пира гонится с roam-эвентом `DeviceOnline` того же пира в окне read→Mount — `tryMount` со снапшотом `IP_old` может перемонтировать шару на старый endpoint после того, как `handleDeviceOnline` уже перемонтировал её на `IP_new`. Снапшот-по-значению этого НЕ закрывает (значение всё равно `IP_old`, т.к. pointee не мутируется), и это не специфично для `tryMount` — тем же образом гонятся `processInitialDevices`/`handleDeviceOnline`. Окно крайне узкое, само-лечится на следующем реконнекте (`processInitialDevices` перемонтирует на актуальный endpoint) или roam'е. Полное закрытие требует атомарного resolve-endpoint+Mount через d.mu/m.mu (кросс-локовый дизайн) — вне минимального scope #61, тот же класс, что device-/share-level add-only gap.
