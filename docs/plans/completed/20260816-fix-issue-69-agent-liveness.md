...
### Task 11: замыкание последних low-находок (➕)

- [x] `markOnlineUnlessDraining` — флаг `draining` проверяется рядом с записью статуса, а не
      только в начале `Register`: между проверкой и записью несколько round-trip'ов, а
      `Hub.Stop` ЖДЁТ этот RPC в `GracefulStop`, так что строка пережила бы sweep
- [x] комментарии больше не ссылаются на несуществующий `recoverIfConnected` (единственная
      документация протокола блокировок — она обязана быть точной)
